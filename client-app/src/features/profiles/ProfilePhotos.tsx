                                    ...
                                    <Card key={photo.id}>
                                        ...
                                        {isCurrentUser &&
                                            <Button.Group fluid widths={2}>
                                                <Button name={photo.id} basic positive content='Main' onClick={(e) => { setTarget(e.currentTarget.name); setMainPhoto(photo); }} loading={loading && target === photo.id} disabled={photo.isMain} />
                                                <Button basic negative content='Trash' name={photo.id} disabled={photo.isMain} onClick={(e) => { deletePhoto(photo); setDeleteTarget(e.currentTarget.name) }} loading={loading && deleteTarget === photo.id} />
                                            </Button.Group>
                                        }
                                    </Card>
                                ))}
